...
export const CaseStudiesShowcase = () => {
  ...
  return <section id="case-studies-showcase" className="py-24 px-6 bg-muted/30">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {caseStudies.map((study, index) => <motion.div key={study.slug} initial={{
          opacity: 0,
          y: 50
        }} whileInView={{
          opacity: 1,
          ...
          once: true
              ...
              <Card className="glass-card overflow-hidden group h-full">
                <div className="relative h-56 overflow-hidden">
                  <img src={study.imageUrl} alt={study.clientName} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                  <div className="absolute inset-0 bg-gradient-to-t from-background/90 via-background/20 to-transparent" />
                </div>
                <CardContent className="p-6">
                  <a href={study.clientUrl} target="_blank" rel="noopener noreferrer" className="text-2xl font-medium text-foreground hover:text-primary transition-colors">
                    {study.clientName}
                  </a>
